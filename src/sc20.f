...
c
      subroutine sc20(y, n, x, p, xg, ng, nl,
     .   coef, tt, pval, v, opt,
     .   pvt, xtx, qr, b, r0, r1, x1, x2, xty, qty,
     .   qraux, work)
      integer n, p, nl, opt
      integer ng
      double precision y(n), x(n,p), xg(n,ng,nl),
     .   coef(ng,p+nl), tt(ng), pval(ng), v(ng)
c
      integer pvt(p + nl)
      double precision xtx(p,p), qr(p+nl, p+nl),
     .   b(p+nl), r0(n), r1(n), x1(n,p), x2(n,nl),
     .   xty(p), qty(p+nl),
     .   qraux(p+nl), work(p+nl,2)
c
